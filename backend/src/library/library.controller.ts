...
export class LibraryController {
    ...
    // ─── Auth-context endpoint (works for both guests and logged-in users) ────

    /**
     * Returns the current user's like state and reading-list status for a book.
     * Guests receive { liked: false, readingStatus: null }.
     */
    @ApiOperation({ summary: 'Get like + reading-list status for a book (requires auth)' })
    @ApiBearerAuth()
    @UseGuards(OptionalJwtAuthGuard)
    @Get('books/:bookId/me-status')
    getBookMeStatus(@Req() req: any, @Param('bookId') bookId: string) {
        const userId: string | undefined = req.user?.sub;
        return this.libraryService.getBookMeStatus(userId, bookId);
    }
    ...
    toggleLike(@Req() req: any, @Param('bookId') bookId: string) {
    ...
    @UseGuards(JwtAuthGuard)
    @Get('me/likes')
    getMyLikes(@Req() req: any, @Query() query: PaginationAndFilterDto) {
    ...
    @UseGuards(JwtAuthGuard)
    @Get('me/reading-list')
    getMyReadingList(@Req() req: any, @Query() query: PaginationAndFilterDto) {
    ...
    addToReadingList(@Req() req: any, @Param('bookId') bookId: string, @Body() body: ReadingStatusBodyDto) {
    ...
    updateReadingListItem(@Req() req: any, @Param('bookId') bookId: string, @Body() body: ReadingStatusBodyDto) {
        return this.libraryService.upsertReadingItem(req.user.sub, bookId, body.status);
    }
    ...
    removeFromReadingList(@Req() req: any, @Param('bookId') bookId: string) {
    ...
    @UseGuards(JwtAuthGuard)
    @Get('me/books')
    searchMyBooks(@Req() req: any, @Query() query: PaginationAndFilterDto) {
    ...
    createComment(@Req() req: any, @Param('bookId') bookId: string, @Body() body: CommentBodyDto) {
    ...
    updateComment(@Req() req: any, @Param('commentId') commentId: string, @Body() body: CommentBodyDto) {
    ...
    @UseGuards(JwtAuthGuard)
    @Get('me/comments')
    getMyComments(@Req() req: any, @Query() query: PaginationAndFilterDto) {
